...
const MapLayers = ({ data }) => {
  ...
  const [[ mapLayers, additionalLayers ], setMapLayers ] = useState([[],[]])
  ...
  useEffect(() => {
    if (data) {
      ...
      layers.push({
        ...
        renderOnLoad: true,
        layer: defaultLayerActions(groupsLayer)
      })
      ...
      // layers.push({
      //   ...groupsLayerDisplayOptions,
      //   label: "TEST",
      //   renderOnLoad: false,
      //   layer: {
      //     add: () => groupsLayer.setStyle({ fillColor: "red", weight: 2 }),
      //     remove: () => groupsLayer.resetStyle()
      //   }
      // })

      setMapLayers([layers, additionalLayers])
    }
  }, [ data ])
  ...
  return <LayerControl layers={ mapLayers } {...{ additionalLayers }} />
}
